...
const createPatient = async (
  ...
  email = commonHelper.isValidEmail(email).toLowerCase();
  if (await isPatientEmailInDb(email))
    throw { Status: 400, error: "An account already exists with this email" };
...
const checkUser = async (email, password) => {
  ...
  else if (await bcryptjs.compare(password, patientInDb.hashedPassword))
    return patientInDb;
  throw { status: 401, error: "Invalid email or password" };
};
